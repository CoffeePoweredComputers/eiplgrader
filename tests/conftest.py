...
def pytest_collection_modifyitems(config, items):  # pylint: disable=unused-argument
    ...
    for item in items:
        ...
        if "infrastructure" in item.nodeid or "adapter" in item.name.lower() or "registry" in item.name.lower():
            item.add_marker(pytest.mark.infrastructure)
        
        # Add edge_cases marker
        if "edge_case" in item.nodeid or "error" in item.name.lower() or "malformed" in item.name.lower():
