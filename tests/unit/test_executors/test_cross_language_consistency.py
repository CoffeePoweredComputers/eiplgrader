...
class TestCrossLanguageConsistency:
    ...
    def _create_test_case_with_types(
        self, language, function_name, parameters, expected, inplace="0"
    ):
        ...
        type_mappings = {
            "java": {
                ...
                "factorial": {"param_types": {"n": "int"}, "expected_type": "int"},
            },
            "cpp": {
                "addNumbers": {
                    "param_types": {"a": "int", "b": "int"},
        ...
        if language in type_mappings and function_name in type_mappings[language]:
            ...
            if language == "c" and function_name == "sumEvenNumbers":
                base_case["parameters"]["size"] = len(
                    base_case["parameters"]["numbers"]
                )
    ...
    def test_add_numbers_consistency(self):
        ...
        if len(results) < 2:
            pytest.skip(
                f"Not enough languages available for comparison. Found: {list(results.keys())}"
            )
        ...
        for language, lang_results in results.items():
            ...
            assert len(lang_results) == len(
                first_results
            ), f"Result count mismatch for {language}"

            for i, (first_result, lang_result) in enumerate(
                zip(first_results, lang_results)
            ):
    ...
    def test_count_vowels_consistency(self):
        ...
        for language, lang_results in results.items():
            ...
            for i, (first_result, lang_result) in enumerate(
                ...
                if first_result["passed"] and lang_result["passed"]:
                    assert (
                        first_result["actual"] == lang_result["actual"]
    ...
    def test_palindrome_consistency(self):
        ...
        for language, executor in self.executors.items():
            ...
            for data in test_data:
                ...
                test_case = self._create_test_case_with_types(
                    language, "isPalindrome", {"s": data["s"]}, expected_value
                )
    ...
    def test_factorial_consistency(self):
        ...
        for language, executor in self.executors.items():
            ...
            for data in test_data:
                test_case = self._create_test_case_with_types(
                    language, "factorial", {"n": data["n"]}, data["expected"]
                )
    ...
    def test_type_inference_vs_explicit_types(self):
        ...
        type_inference_langs = [
            lang
            for lang in self._get_type_inference_languages()
            if lang in self.executors
            and self.executors[lang]
            and self._check_compiler_available(lang)
        ]

        static_type_langs = [
            lang
            for lang in self._get_static_type_languages()
            if lang in self.executors
            and self.executors[lang]
            and self._check_compiler_available(lang)
        ]
        ...
        for lang in type_inference_langs:
            try:
                ...
                result = self.executors[lang].execute_test(
                    code_sample, test_case_inference
                )
        ...
        if all_results:
            ...
            assert all(
                r == expected for r in all_results
    ...
    def test_error_handling_consistency(self):
        ...
        for language, executor in self.executors.items():
            if (
                not executor
                or not self._check_compiler_available(language)
                or language not in invalid_codes
            ):
                continue

            test_case_typed = self._create_test_case_with_types(
                language, "brokenFunc", {"x": 1}, 2
            )
        ...
        for language, result in error_results.items():
            assert not result[
                "passed"
            ], f"{language} should have failed for invalid code"
    ...
    def test_empty_input_handling(self):
        ...
        if successful_results:
            assert all(
                r == 0 for r in successful_results
