...
class TestLanguageExecutorBase:
    ...
    def test_validate_types_provided_success(self):
        ...
        class MockExecutor(LanguageExecutor):
            def prepare_code(self, code, test_case): pass
            def execute_test(self, code, test_case): pass  
...
class TestCompiledLanguageExecutor:
    ...
    @patch('eiplgrader.languages.executors.base_executors.CompiledLanguageExecutor.compile')
    @patch('eiplgrader.languages.executors.base_executors.CompiledLanguageExecutor.prepare_code')
    @patch('subprocess.run')
    def test_execute_test_compilation_failure(self, mock_run, mock_prepare, mock_compile):
    ...
    @patch('eiplgrader.languages.executors.base_executors.CompiledLanguageExecutor.prepare_code')
    @patch('subprocess.run')
    def test_execute_test_exception_handling(self, mock_run, mock_prepare, mock_compile):
...
class TestInterpretedLanguageExecutor:
    ...
    def setup_method(self):
        ...
        self.executor = MockInterpretedExecutor(
            interpreter_cmd=self.interpreter_cmd,
            file_ext=self.file_ext
        )
    ...
    @patch('eiplgrader.languages.executors.base_executors.InterpretedLanguageExecutor.prepare_code')
    @patch('subprocess.run')
    ...
    def test_execute_test_json_parsing_edge_cases(self, mock_run, mock_prepare):
        ...
        mock_run.return_value = Mock(returncode=0, stdout='  {"value": 42}  \n', stderr="")
        test_case = {"parameters": {}, "expected": {"value": 42}}
        result = self.executor.execute_test("code", test_case)
        assert result["actual"] == {"value": 42}
