...
class TestJavaAdapter:
    ...
    return result;
        ...
        normalized = self.adapter.normalize_code(code)
        expected = "public static int add(int a, int b) { int result = a + b; return result; }"
        assert normalized == expected
...
class TestCompiledLanguageAdapters:
    ...
    @pytest.mark.parametrize("adapter_class,expected_name,expected_compile_cmd", [
        (CAdapter, "c", ["gcc"]),
        (CppAdapter, "cpp", ["g++"]),
        (GoAdapter, "go", ["go", "build"]),
        (HaskellAdapter, "haskell", ["ghc"]),
    ])
    def test_compiled_adapter_configs(self, adapter_class, expected_name, expected_compile_cmd):
...
class TestAdapterCodeExtractionPatterns:
    ...
    @pytest.mark.parametrize("adapter_class,language_name", [
    ...
    def test_extract_code_empty_handling(self, adapter_class, language_name):
    ...
    @pytest.mark.parametrize("adapter_class", [
        PythonAdapter, JavaScriptAdapter, JavaAdapter, 
        CAdapter, CppAdapter, GoAdapter, HaskellAdapter
    ])
