    ...
    def test_error_handling_resilience_python(self):
        ...
        for scenario in error_scenarios:
            tester = CodeTester(
                ...
                function_name=(
                    "test_func" if "test_func" in scenario["code"] else "bad_function"
                ),
                language="python",
            )
        ...
        for result in results:
            ...
            assert (
                result["errors"] > 0 or result["failures"] > 0
            ), f"{result['description']} should have errors or failures"
...
"""
        ...
        invalid_scenarios = [
            ...
            {"test_cases": ["not a dict"], "should_raise": ValueError},
            # Missing parameters key
            {"test_cases": [{"expected": 3}], "should_raise": ValueError},
...
""",
                "test_cases": [
                    {"parameters": {"delay": 1}, "expected": 1, "timeout": 0.5}
                ],
...
""",
        ...
        for scenario in cgbg_failure_scenarios:
            ...
            if scenario["description"] == "Multiple functions generated":
                ...
                ), f"Multiple functions scenario should work"
                ...
                assert (
                    not result.was_successful()
                ), f"{scenario['description']} should fail"
                assert result.errors > 0 or result.failures > 0
