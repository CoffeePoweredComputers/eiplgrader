    ...
    def test_default_timeout_values(self, mock_run):
        ...
        try:
            with patch.object(compiled_executor, 'prepare_code', return_value="test"):
                with patch.object(compiled_executor, 'compile', return_value=(True, "/tmp/test", "")):
        ...
        try:
            with patch.object(interpreted_executor, 'prepare_code', return_value="test"):
                test_case = {"parameters": {}, "expected": ""}
    ...
    def test_timeout_precision(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            def timeout_after_delay(*args, **kwargs):  # pylint: disable=unused-argument
                ...
                time.sleep(min(timeout + 0.1, 1.0))  # Sleep slightly longer than timeout, but cap at 1 second for test speed
            ...
            try:
                with patch.object(executor, 'prepare_code', return_value="test"):
                    test_case = {"parameters": {}, "expected": "", "timeout": 0.5}  # Short timeout for test
            ...
            try:
                ...
                with patch('subprocess.run') as mock_run:
                    mock_run.return_value = Mock(
                        returncode=0, 
                        stdout=f"result_{executor_id}", 
                        stderr=""
                    )
                    
                    with patch.object(executor, 'prepare_code', return_value=f"print({executor_id})"):
                        test_case = {"parameters": {}, "expected": f"result_{executor_id}"}
                        result = executor.execute_test("test", test_case)
    ...
    def test_executor_reuse_safety(self):
        ...
        try:
            ...
            with patch('subprocess.run') as mock_run:
                ...
                for i in range(5):
                    with patch.object(executor, 'prepare_code', return_value=f"print({i})"):
...
class TestErrorRecoveryAndCleanup:
    ...
    def test_cleanup_after_compilation_failure(self):
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.return_value = Mock(returncode=1, stderr="Compilation failed", stdout="")
            
            executor = MockCompiledExecutor(["gcc"], ["./"], ".c")
            temp_dir = executor.temp_dir
            
            try:
                with patch.object(executor, 'prepare_code', return_value="invalid code"):
                    test_case = {"parameters": {}, "expected": ""}
    ...
    def test_cleanup_after_runtime_failure(self):
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.return_value = Mock(returncode=1, stderr="Runtime error", stdout="")
            
            executor = MockInterpretedExecutor(["python3"], ".py")
            temp_dir = executor.temp_dir
            
            try:
                with patch.object(executor, 'prepare_code', return_value="raise Exception()"):
                    test_case = {"parameters": {}, "expected": ""}
    ...
    def test_cleanup_during_exception_handling(self):
        ...
        try:
            ...
            with patch('subprocess.run') as mock_run:
                ...
                with patch.object(executor, 'prepare_code', return_value="test"):
                    with patch.object(executor, 'compile', return_value=(True, "/tmp/test", "")):
                        test_case = {"parameters": {}, "expected": ""}
                        result = executor.execute_test("test", test_case)
    ...
    def test_forced_cleanup_with_context_manager(self):
        ...
        with ExecutorContextManager(CompiledLanguageExecutor, ["gcc"], ["./"], ".c") as executor:
            temp_dir = executor.temp_dir
            assert os.path.exists(temp_dir)
    ...
    def test_concurrent_executor_resource_usage(self):
        ...
        def create_executor_with_files(executor_id):
            try:
                ...
                executor_results.append((executor_id, temp_dir, os.path.exists(test_file)))
                executor.cleanup()
                
                # Verify cleanup
                executor_results.append((executor_id, temp_dir, os.path.exists(temp_dir)))
        ...
        creation_results = [r for r in executor_results if r[2] is True and r[1] != "error"]
        cleanup_results = [r for r in executor_results if r[2] is False and r[1] != "error"]
