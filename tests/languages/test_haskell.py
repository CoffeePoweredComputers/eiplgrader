...
add x y = x + y"""
    ...
    def test_validate_syntax_invalid(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=1, 
            stderr="test.hs:3:1: error: parse error on input 'add'"
        )
...
isEven n = n `mod` 2 == 0"""
        ...
        assert 'if result then "true" else "false"' in prepared or "if result then \"true\" else \"false\"" in prepared
...
range a b = [a..b]"""
    ...
    def test_execute_test_compilation_error(self, mock_mkdtemp, mock_run):
        ...
        compile_result = MagicMock(
            returncode=1, 
            stderr="test.hs:2:1: error: parse error on input 'add'"
        )
...
add x y = """  # Invalid syntax
    ...
    def test_execute_test_runtime_error(self, mock_mkdtemp, mock_run):
        ...
        exec_result = MagicMock(
            returncode=1, 
            stdout="", 
            stderr="*** Exception: divide by zero"
        )
        mock_run.side_effect = [compile_result, exec_result]
