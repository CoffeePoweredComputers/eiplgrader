...
function add_mixed($a, $b) {
    return $a + $b;
        ...
        test_cases = [
            {
                "function_name": "add_mixed",
                "parameters": {"a": "5", "b": 3},
                "expected": 8,
            },
            {
                "function_name": "add_mixed",
                "parameters": {"a": 2.5, "b": 1.5},
                "expected": 4.0,
            },
            {
                "function_name": "add_mixed",
        ...
        for test_case in test_cases:
            ...
            assert result[
                "passed"
            ], f"Test failed for {test_case}: {result.get('error')}"
...
function get_value($data, $key) {
    return isset($data[$key]) ? $data[$key] : null;
        ...
        test_case = {
            ...
            "parameters": {"data": {"name": "John", "age": 30}, "key": "name"},
            "expected": "John",
        }
        result = executor.execute_test(code, test_case)
        assert result["passed"], f"Test failed: {result.get('error', 'Unknown error')}"
    ...
    def test_prompt_generation_types(self, adapter):
        ...
        cgbg_prompt = adapter.generate_prompt(
            "multiply two numbers and return the result", "multiply", gen_type="cgbg"
        )
    ...
    def test_no_echo_enforcement(self, adapter):
        ...
        prompt = adapter.generate_prompt("calculate the sum", "sum", gen_type="cgbg")
