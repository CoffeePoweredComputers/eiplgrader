...
let rec map f lst =
  ...
  | h::t -> (f h) :: (map f t)"""
    ...
    def test_validate_syntax_timeout(self, mock_run):
        ...
        valid, error = self.adapter.validate_syntax("let infinite = while true do () done")
        assert valid is False
...
class TestOcamlExecutor:
    ...
    def test_compile_failure(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=1,
            stderr="Error: Unbound value foo",
            stdout=""
        )
    ...
    def test_execute_test_runtime_error(self, mock_mkdtemp, mock_run):
        ...
        compile_result = Mock(returncode=0, stderr="", stdout="")
        exec_result = Mock(returncode=1, stderr="Exception: Division_by_zero", stdout="")
