...
class TestComprehensiveSuite:
    ...
    def test_suite_integration(self):
        ...
        for test_dir in test_dirs:
            assert (project_root / test_dir).exists(), f"Missing test directory: {test_dir}"
    ...
    def test_language_coverage(self):
        ...
        supported_languages = [
            "python", "javascript", "java", "c", "cpp", "go", "haskell"
        ]
    ...
    def test_agent2_infrastructure_robustness(self):
        ...
        for language in language_registry.list_languages():
            ...
            if language in ["python", "javascript", "go"]:
                assert len(config.run_command) > 0, f"{language} should have run_command"
            elif language in ["c", "cpp", "java", "haskell"]:
                # Compiled languages might have empty run_command but should have compile_command
                assert config.compile_command is not None and len(config.compile_command) > 0, \
...
def run_comprehensive_suite():
    ...
    parser.add_argument("--category", choices=["type-system", "infrastructure", "edge-cases", "all"], 
