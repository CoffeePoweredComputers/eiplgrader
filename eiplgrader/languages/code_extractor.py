...
class CodeExtractor:
    ...
    def extract_with_debug(
        self, llm_response: str, language_or_spec
    ...
    def _extract_by_patterns(self, response: str, spec: LanguageSpec) -> List[str]:
        ...
        # Handle the function_patterns field which can be either:
        # 1. A single FunctionPatterns object (from LanguageSpec)
        # 2. A list of pattern strings (from _create_minimal_spec)
        patterns_to_use = []

        if hasattr(spec, "_legacy_function_patterns"):
            # This is a minimal spec with legacy pattern list
            patterns_to_use = spec._legacy_function_patterns  # type: ignore
        elif spec.function_patterns is not None:
            # This is a proper LanguageSpec with FunctionPatterns object
            patterns_to_use = [spec.function_patterns.definition_regex]
    ...
    def _process_extracted_code(
        self, matches: List[str], spec: LanguageSpec
    ) -> List[str]:
        ...
        for match in matches:
            ...
            if spec.header_requirements.required_imports and not any(
                header in code for header in ["#include", "import", "using"]
            ):
                headers = "\n".join(spec.header_requirements.required_imports)
                code = headers + "\n\n" + code
    ...
    def _create_minimal_spec(self, language: str) -> LanguageSpec:
        ...
        from .spec import HeaderRequirements
        ...
        spec = LanguageSpec(
            ...
            function_patterns=None,  # Will be handled via _legacy_function_patterns
            header_requirements=HeaderRequirements(),
        )

        # Store legacy patterns as a private attribute for backward compatibility
        spec._legacy_function_patterns = function_patterns_map.get(language, [])  # type: ignore
...
class CodeProcessor:
    ...
    def process(code: str, spec: LanguageSpec) -> str:
        ...
        # Header injection
        if spec.header_requirements.required_imports and spec.name in ["c", "cpp"]:
    ...
    def _inject_headers(code: str, spec: LanguageSpec) -> str:
        ...
        if spec.header_requirements.required_imports:
            headers = "\n".join(spec.header_requirements.required_imports)
            if headers not in code:
                return headers + "\n\n" + code
        return code
