...
class CompilerValidator(ValidationStrategy):
    ...
    def _validate_with_command(
        self, code: str, command: List[str]
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=self.spec.file_extensions[0], delete=False
            ) as f:
            ...
            try:
                ...
                final_command = [
                    temp_path if arg == "{file}" else arg for arg in command
                ]
                ...
                result = subprocess.run(
                    final_command, capture_output=True, text=True, timeout=30
                )
                ...
                if result.returncode == 0:
                    ...
                    error_msg = error_processor.process_compiler_error(
                        result.stderr or result.stdout, self.spec.name
                    )
    ...
    def _validate_with_compiler(self, code: str) -> Tuple[bool, Optional[str]]:
        ...
        if self.spec.compile_command is None:
            return True, None
    ...
    def _prepare_code_for_validation(self, code: str) -> str:
        ...
        if (
            getattr(self.spec, "requires_main_wrapper", False)
            and "main" not in code.lower()
        ):
            if self.spec.name == "c":
                prepared += "\n\nint main() { return 0; }"
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    error_msg = error_processor.process_error(
                        result.stderr.strip(), "javascript", temp_path
                    )
                    return False, error_msg
    ...
    def _validate_generic(self, code: str) -> Tuple[bool, Optional[str]]:
        ...
        if hasattr(self.spec, "syntax_conventions") and self.spec.syntax_conventions:
            ...
            if conventions.block_start and conventions.block_end:
                if code.count(conventions.block_start) != code.count(
                    conventions.block_end
                ):
                    issues.append(
                        f"Unmatched {conventions.block_start}/{conventions.block_end}"
                    )
...
class CustomValidator(ValidationStrategy):
    ...
    def validate(self, code: str) -> Tuple[bool, Optional[str]]:
        ...
        if self.validation_func:
            result = self.validation_func(code)
        ...
        try:
            ...
            try:
                ...
                if any(
                    ...
                    for msg in ["no such table", "no such column"]
                ):
                    return True, None
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    error_msg = error_processor.process_error(
                        result.stderr.strip(), "bash", temp_path
                    )
                    return False, error_msg
...
class CompositeValidator(ValidationStrategy):
    ...
    def __init__(
        self, validators: List[ValidationStrategy], stop_on_first_error: bool = True
    ):
...
def create_validator(
    spec: LanguageSpec, custom_validation_func=None
) -> ValidationStrategy:
...
def create_composite_validator(
    spec: LanguageSpec, strategies: List[str], custom_validation_func=None
) -> CompositeValidator:
    """Create a composite validator with multiple strategies."""
    validators: List[ValidationStrategy] = []
