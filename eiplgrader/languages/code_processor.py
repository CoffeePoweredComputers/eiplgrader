...
class ClassWrappingProcessor(CodeProcessor):
    ...
    def process(self, code: str, language: str, **kwargs) -> str:
        ...
        if language == "java" and re.search(r"(public\s+)?class\s+\w+", code):
            return code
        elif language in ["kotlin", "scala"] and re.search(r"(class|object)\s+\w+", code):
            return code
...
class LanguageCodeProcessor:
    ...
    def __init__(self):
        ...
        self.language_processors = {
            ...
            "haskell": ["type_signature", "module_wrapper", "whitespace_normalization", "comment_strip"],
    ...
    def process_code(self, code: str, language: str, **kwargs) -> str:
        ...
        if not code or language not in self.language_processors:
            ...
            return self.processors["whitespace_normalization"].process(code, language, **kwargs)
        ...
        sorted_processors = sorted(
            [(name, self.processors[name]) for name in processor_names if name in self.processors],
    ...
    def process_with_debug(self, code: str, language: str, **kwargs) -> Dict[str, Union[str, List[str]]]:
        ...
        """
        if not code or language not in self.language_processors:
            processed = self.processors["whitespace_normalization"].process(code, language, **kwargs)
            return {
                "original_code": code,
...
def create_custom_processor(name: str, process_func: Callable[[str, str], str], 
                          applies_to: List[str], priority: int = 0) -> CodeProcessor:
