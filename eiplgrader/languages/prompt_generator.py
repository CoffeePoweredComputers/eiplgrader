...
class PromptGenerator:
    ...
    def _load_template(self, gen_type: str, language_spec: LanguageSpec) -> str:
        ...
        override_template = getattr(
            language_spec.template_overrides, f"{gen_type}_template", None
        )
        if override_template is not None:
            return str(override_template)
    ...
    def _build_context(
        ...
        **kwargs,
        ...
        context = {
            ...
            "student_model_description": self._get_student_model_description(
                language_spec
            ),
            # Function declaration specifics
            "function_declaration_type": self._get_function_declaration_type(
                language_spec
            ),
            # Multi-version generation
            "multi_version_instruction": self._get_multi_version_instruction(
                kwargs.get("num_to_gen", 1)
            ),
        }
        ...
        if gen_type == "cgbg":
            context.update(
                self._build_cgbg_context(language_spec, function_name, **kwargs)
            )
        elif gen_type == "redef":
            context.update(
                self._build_redef_context(language_spec, function_name, **kwargs)
            )
    ...
    def _build_cgbg_context(
        ...
        **kwargs,  # pylint: disable=unused-argument
        ...
        """
        return {
            "language_specific_instructions": self._get_cgbg_instructions(
                language_spec
            ),
            "function_example": self._get_cgbg_function_example(
                language_spec, function_name
            ),
    ...
    def _build_redef_context(
        ...
        return {
            ...
            "function_signature": self._format_function_signature(
                language_spec, function_name, params
            ),
            "assumptions": assumptions,
            "signature_preservation_note": self._get_signature_preservation_note(
                language_spec
            ),
            "function_example": self._get_redef_function_example(
                language_spec, function_name, params
            ),
    ...
    def _apply_overrides(
        ...
        gen_type: str,  # pylint: disable=unused-argument
    ) -> Dict[str, Any]:
    ...
    def _get_cgbg_function_example(
        self, language_spec: LanguageSpec, function_name: str
    ) -> str:
...
Do not include any semicolons at the end of the query.""",
    ...
    def _format_function_signature(
        self, language_spec: LanguageSpec, function_name: str, params: str
    ) -> str:
    ...
    def _get_redef_function_example(
        self, language_spec: LanguageSpec, function_name: str, params: str
    ) -> str:
...
constraints."""
    ...
    def validate_template(self, template_path: Path) -> bool:
        ...
        required_vars = {"language_display_name", "code_block_tag"}
        ...
        try:
            ...
            raise PromptValidationError(f"Template formatting error: {e}") from e
