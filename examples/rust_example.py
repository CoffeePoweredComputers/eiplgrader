...
generated_code = code_generator.generate_code(
    ...
    params="a, b",
    assumptions="a and b are i32 integers",
    model="gpt-4o",
    language="rust"
)
...
code_tester = CodeTester(generated_code['code'], test_cases, language="rust")
...
# Test the generated code
code_tester = CodeTester(generated_code['code'], test_cases, function_name="sum_nums", language="rust")
test_result = code_tester.run_tests()
print(test_result, end="\n\n")
...
# Test the generated code
code_tester = CodeTester(generated_code['code'], test_cases, function_name="sums_numbers", language="rust")
test_result = code_tester.run_tests()
print(f"Test result: {test_result}", end="\n\n")
...
generated_code = code_generator.generate_code(
    "iterates through a vector of integers at each point adding the current number to a sum var and, at the end, returning that var.",
    segmentation_few_shot_file="segmentation_few_shot.json",
    params="vec",
    assumptions="vec is a vector of i32 integers",
    model="gpt-4o",
    language="rust"
)

test_cases = [
    {"parameters": {"vec": [1, 2, 3]}, "expected": 6},
    {"parameters": {"vec": [5, 7, 8]}, "expected": 20},
    {"parameters": {"vec": [-1, 1, 0]}, "expected": 0},
]
