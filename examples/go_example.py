...
code_tester = CodeTester(generated_code['code'], test_cases, language="go")
...
# Test the generated code
code_tester = CodeTester(generated_code['code'], test_cases, function_name="sum_nums", language="go")
test_result = code_tester.run_tests()
print(test_result, end="\n\n")
...
# Test the generated code
code_tester = CodeTester(generated_code['code'], test_cases, function_name="sums_numbers", language="go")
test_result = code_tester.run_tests()
print(f"Test result: {test_result}", end="\n\n")
...
generated_code = code_generator.generate_code(
    "iterates through a slice of integers at each point adding the current number to a sum var and, at the end, returning that var.",
    segmentation_few_shot_file="segmentation_few_shot.json",
    params="slice",
    assumptions="slice is a slice of integers",
    model="gpt-4o",
    language="go"
)

test_cases = [
    {"parameters": {"slice": [1, 2, 3]}, "expected": 6},
    {"parameters": {"slice": [5, 7, 8]}, "expected": 20},
    {"parameters": {"slice": [-1, 1, 0]}, "expected": 0},
]
